...
const EditUserDialog = ({ selectedUser, onSuccess }: UserListHeaderProps) => {
  ...
  const [form, setForm] = useState<UserForm>({
    ...
    password: "",
    confirmPassword: "",
    role: selectedUser?.role as UserForm["role"],
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!result.success) {
      ...
      try {
        ...
        const response = await fetch(
          `${import.meta.env.VITE_API_SERVER_URL}/api/profiles/${selectedUser?.id}`,
          {
            method: "PUT",
            headers: {
        ...
        setLoading(false);
        console.error("Failed to edit user request: ", error);
        setDialogProp({
          type: "error",
          message: `Failed to Edit Account. \n Please try again`,
        });
    ...
    setForm({
      ...
      password: "",
      confirmPassword: "",
      role: form.role as UserForm["role"],
    });
  };
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="[&>button]:hidden sm:max-w-[600px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Fill in the fields you want to update for this user.
            </DialogDescription>
          </DialogHeader>
